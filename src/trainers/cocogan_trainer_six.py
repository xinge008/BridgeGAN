...
class COCOGANTrainer_six(nn.Module):
  def __init__(self, hyperparameters):
    super(COCOGANTrainer_six, self).__init__()
  ...
  def gen_update(self, images_a, images_b, hyperparameters):
    """
    :param images_a: 3 channels
    :param images_b: 6 channels
  ...
  def save(self, snapshot_prefix, iterations):
    gen_filename = '%s_gen_%08d_smo_clip.pkl' % (snapshot_prefix, iterations + 1)
    dis_filename = '%s_dis_%08d_smo_clip.pkl' % (snapshot_prefix, iterations + 1)
  ...
  def parallel(self, gpu_num):
    self.dis = nn.DataParallel(self.dis, device_ids=range(gpu_num), output_device=0)
    self.gen = nn.DataParallel(self.gen, device_ids=range(gpu_num), output_device=0)
