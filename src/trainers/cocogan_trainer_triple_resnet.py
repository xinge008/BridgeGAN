...
class COCOGANTrainer_triple_res(nn.Module):
  ...
  def gen_update(self, images_a, images_b,images_c, hyperparameters):
    ...
    # test_coral = self.gen.forward_coral(images_a)
    # print "coral_size: ", test_coral.size()

    x_ab_1, x_ab_2 = self.resnet50(x_ab)
    x_gtb_1, x_gtb_2 = self.resnet50(images_b)
    ...
    for it, (out_a, out_b, out_c, out_d) in enumerate(itertools.izip(outs_a, outs_b, outs_c, outs_d)):
      ...
      all_ones = Variable(torch.from_numpy(np.random.uniform(0.8,1.0,size=(outputs_a.size())))).float().cuda()
    ...
    # print shared
    semantic_ba_b = torch.mean(torch.abs(x_ab_1-x_gtb_1) +torch.abs(x_bab_1-x_gtb_1) )
    # semantic_ba_b_2 = torch.mean(torch.abs(x_ab_2-x_gtb_2))
    semantic_bab_b = torch.mean(torch.abs(torch.abs(x_ab_2-x_gtb_2) + x_bab_2-x_gtb_2))
    # semantic_bab_b_2 = torch.mean(torch.abs(x_bab_2-x_gtb_2))

    #>>>>>>>>>>>>>>>>>>>>>>compute the color histogram<<<<<<<<<<<<<<<<<<<<<<

    # colorhistc_ab_gtb = torch.mean(torch.abs(torch.histc(x_ab.data.cpu(), bins=64, min=-1, max=1) - torch.histc(images_b.data.cpu(), bins=64, min=-1, max=1)) +
    #                                torch.abs(torch.histc(x_bab.data.cpu(), bins=64, min=-1, max=1) - torch.histc(images_b.data.cpu(), bins=64, min=-1, max=1)))
    #
    #
    # print colorhistc_ab_gtb
    # print ">>>>><<<<<<<<<"
    ...
    total_loss = hyperparameters['gan_w'] * (ad_loss_a + ad_loss_b) + \
                 ...
                 10 * (semantic_ba_b + semantic_bab_b) + \
    ...
    #              + colorhistc_ab_gtb
    # 10 * (semantic_ba_b + semantic_bab_b) + \
      ## without cycle-consistency loss and perceptual loss
    # total_loss = hyperparameters['gan_w'] * (ad_loss_a + ad_loss_b) + \
    #              0.1 *  (ad_loss_c + ad_loss_d) + \
    #              hyperparameters['ll_direct_link_w'] * (ll_loss_a + ll_loss_b +ll_loss_c) + \
    #              hyperparameters['kl_direct_link_w'] * (enc_loss + enc_loss + enc_loss)
  ...
  def dis_update(self, images_a, images_b, images_c, hyperparameters):
    ...
    for it, (this_a, this_b, this_c, this_d) in enumerate(itertools.izip(res_a, res_b, res_c, res_d)):
      ...
      tmp_all1 = torch.from_numpy(np.random.uniform(0.8,1.2, size=out_true_n))
      # print tmp_all1
      # print out_true_a
      all1 = Variable(tmp_all1).float().cuda()
      # all0 = Variable(torch.zeros((out_fake_n)).cuda(self.gpu))
      tmp_all0 = torch.from_numpy(np.random.uniform(0.0, 0.3, size=out_fake_n))
  ...
  def save(self, snapshot_prefix, iterations):
    gen_filename = '%s_gen_%08d_smo.pkl' % (snapshot_prefix, iterations + 1)
    dis_filename = '%s_dis_%08d_smo.pkl' % (snapshot_prefix, iterations + 1)
