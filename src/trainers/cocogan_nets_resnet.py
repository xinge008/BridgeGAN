...
class COCODis_resnet(nn.Module):
  def __init__(self, params):
    super(COCODis_resnet, self).__init__()
    ch = params['ch']
    input_dim_a = params['input_dim_a']
    input_dim_b = params['input_dim_b']
  ...
  def _make_net(self, ch, input_dim, n_layer):
    ...
    tch = ch
    for i in range(1, n_layer-1): # for patchGAN (10x6)
      model += [LeakyReLUConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1)] # 8
      tch *= 2
    model += [nn.Conv2d(tch, 1, kernel_size=1, stride=1, padding=0)]  # 1
    return nn.Sequential(*model)
...
class COCOResGen_resnet(nn.Module):
  def __init__(self, params):
    super(COCOResGen_resnet, self).__init__()
    input_dim_a = params['input_dim_a']
    input_dim_b = params['input_dim_b']
    ch = params['ch']
...
def run():
  ...
  COCOResGen_test = COCOResGen_resnet(params).cuda()
