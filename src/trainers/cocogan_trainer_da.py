...
class COCOGANDAContextTrainer(nn.Module):
  def __init__(self, hyperparameters):
    ...
    exec( 'self.dis = %s(dis_ch, output_dim_a, output_dim_b)' % hyperparameters['dis']) # Dis
    exec( 'self.gen = %s(gen_ch, input_dim_a, output_dim_a, input_dim_b, output_dim_b)' % hyperparameters['gen']) # VAE
  ...
  def _create_xy_image(self, width=32):
    # normalizad x and y coordinates, range(-1, 1)
    coordinates = list(itertools.product(range(width), range(width))) # return ((x,y) for x in A for y in B)
    arr = (np.reshape(np.asarray(coordinates), newshape=[width, width, 2]) - width/2 ) / (width/2)
    new_map = np.transpose(np.float32(arr), [2, 0, 1]) # (3, width, width)
