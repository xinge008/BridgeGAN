...
class COCODis(nn.Module):
  def __init__(self, params):
    ...
    # self.model_A = nn.DataParallel(self.model_A)
...
class COCOResGen(nn.Module):
  def __init__(self, params):
    ...
    # self.encode_A = nn.DataParallel(self.encode_A)
    # self.encode_B = nn.DataParallel(self.encode_B)
    # self.enc_shared = nn.DataParallel(self.enc_shared)
    # self.dec_shared = nn.DataParallel(self.dec_shared)
    # self.decode_B = nn.DataParallel(self.decode_B)
    # self.decode_A = nn.DataParallel(self.decode_A)



  def forward(self, x_A, x_B):
    # print x_A.data.cpu()[0,0,0,:]
    # print ">>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<,"
    out = torch.cat((self.encode_A(x_A), self.encode_B(x_B)), 0) #
    # print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
    # print out.data.cpu()[0,0,0,:]
    shared = self.enc_shared(out)
    # print "++++++++++++++++++++++++++++++++++++++"
    # print shared.data.cpu()[0,0,0,:]
...
def run():
  ...
  COCOResGen_test = COCOResGen(params).cuda()
