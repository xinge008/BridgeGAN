...
class INSResBlock(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, dropout=0.0):
    ...
    self.model.apply(gaussian_weights_init) # another initialization method
...
class GaussianVAE2D(nn.Module):
  ...
  def sample(self, x):
    mu = self.en_mu(x)
    sd = self.softplus(self.en_sigma(x))
    # torch.randn is sampled from a normal distribution with zero mean and variance of one
