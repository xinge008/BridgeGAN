...
"""
from cocogan_nets_resnet import *
import numpy as np
from init import *
...
class COCOGANTrainer_resnet(nn.Module):
  def __init__(self, hyperparameters):
    super(COCOGANTrainer_resnet, self).__init__()
    ...
    self.ll_loss_criterion_resnet = torch.nn.L1Loss()
  ...
  def gen_update(self, images_a, images_b, images_c, hyperparameters):
    ...
    # print x_ab.size()
    # print x_bab.size()
    resnet50_ab = self.resnet50(x_ab)
    resnet50_bab = self.resnet50(x_bab)
    # print resnet50_ab.size()
    resnet50_c = self.resnet50(images_c)
    ...
    # semantic perception loss
    semantic_ab_c = torch.mean(torch.abs(resnet50_ab - resnet50_c))
    semantic_bab_c = torch.mean(torch.abs(resnet50_bab - resnet50_c))
    # print semantic_bab_c
    ...
    total_loss = hyperparameters['gan_w'] * (ad_loss_a + ad_loss_b) + \
                 ...
                 10 * (semantic_ab_c + semantic_bab_c) + \
  ...
  def parallel(self):
    self.dis = nn.DataParallel(self.dis, device_ids=range(4), output_device=3)
    self.gen = nn.DataParallel(self.gen, device_ids=range(4), output_device=3)
    self.dis_opt = nn.DataParallel(self.dis_opt, device_ids=range(4), output_device=3)
    self.gen_opt = nn.DataParallel(self.gen_opt, device_ids=range(4), output_device=3)
